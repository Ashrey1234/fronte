...
//     setMessage({ text: 'Feedback is required for approval', severity: 'error' });
//     return;
//   }
...
//         setMessage({ text: 'Certificate downloaded successfully', severity: 'success' });
//       })
//       .catch(err => setMessage({ text: err.message || 'Failed to download certificate', severity: 'error' }));
//   };
...
//     const payload = {
//       level: selectedApplication.level,
//       nationality: selectedApplication.nationality,
//       research_type: selectedApplication.research_type,
//       application_id: selectedApplication.id
//     };

//     fetch(`${API_BASE_URL}/api/payments/simple/`, {
//       method: "POST",
//       headers: getAuthHeaders(),
...
//         setMessage({ text: "Payment created successfully", severity: "success" });
//       })
...
//   const getStatusColor = (status) => { switch (status) { case 'Pending': return 'warning'; case 'Approved': return 'success'; case 'Rejected': return 'error'; case 'Draft': return 'default'; default: return 'default'; }};
...
//                     <Typography>{selectedApplication.level || 'N/A'}</Typography>
//                   </Grid>
//                   <Grid item xs={12} sm={6}>
//                     <Typography variant="subtitle2">Nationality</Typography>
//                     <Typography>{selectedApplication.nationality || 'N/A'}</Typography>
//                   </Grid>
...
const OfficerDashboard = () => {
  ...
  const handleCreatePayment = () => {
    ...
    const payload = {
      level: selectedApplication.payment_record?.level,
      nationality: selectedApplication.payment_record?.nationality,
      research_type: selectedApplication.research_type,
      application_id: selectedApplication.id
    };
    ...
    fetch(`${API_BASE_URL}/api/payments/simple/`, {
      ...
      .then(async res => {
        ...
        if (!res.ok) throw new Error(data.error || "Payment creation failed");
        setMessage({ text: "Payment created successfully", severity: "success" });
        fetchApplications(); // refresh to include payment info
      })
      .catch(err => setMessage({ text: err.message, severity: "error" }));
  };
